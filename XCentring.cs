...
/// </summary>
public class XCentring : MonoBehaviour
{
    // global frames count
    ...
    void FixedUpdate()
    {
...
#endif
        //if (globalFrames <= 12) { nspeed = init_speed; }
        //else { nspeed *= (1 - 0.005f*Mathf.Sign(bee.left_response + bee.right_response-200f)); }

        //SelfMove(bee.left_response, bee.right_response, nspeed);
        ...
        if (globalFrames <= 30)
        {
            ...
            if (globalFrames > 28)
            {
                Rinit += 0.25f * (bee.left_response + bee.right_response);
            }
            SelfMove(bee.left_response, bee.right_response, nspeed);
        }
        float alpha = 0.05f;
        ...
        {
            //nspeed = init_speed;
            nspeed += alpha * Mathf.Sign(Rinit - 0.5f * bee.left_response - 0.5f * bee.right_response);
            SelfMove(bee.left_response, bee.right_response, nspeed);
        }

        print(System.String.Format("speed: {0:F}", nspeed));
        string result1 = @"F:\xshape5.txt";
    ...
    void SelfMove(float lres, float rres ,float speed)
    {
        // tranlate in z-axis
        float translate;
        ...
        float forward_move = speed * Time.deltaTime;
       // if (globalFrames <= 12) { speed = init_speed; forward_move = speed * Time.deltaTime; }
       // else {  speed *= (400f/ (lres + rres)); forward_move = speed * Time.deltaTime; }
    ...
    public class HoneyBee
    {
        ...
        public void AngularVelocityEstimator(float[,,] leftImgSeq, float[,,] rightImgSeq, int t)
        {
            ...
            print(System.String.Format("0.5 *(ReL + ReR): {0:F} ",0.5* (left_response + right_response)));
